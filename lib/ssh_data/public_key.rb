module SSHData::PublicKey
  ...
  def self.parse(key)
    ...
    if algo.nil? || b64.nil?
      raise SSHData::DecodeError, "bad public key format"
    end

    raw = Base64.decode64(b64)
    data, read = SSHData::Encoding.decode_public_key(raw)
  ...
  def self.from_data(data)
    ...
    when ALGO_ED25519
      ...
      raise SSHData::DecodeError, "unkown algo: #{data[:algo].inspect}"
    end
  end
end
